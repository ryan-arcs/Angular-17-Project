...
@Component({
  selector: 'app-c4e-left-sidebar',
  standalone: true,
  imports: [RouterLink, FormsModule, NgbTooltipModule,NgbModalModule],
...
export class LeftSidebarComponent {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.tableNameSubscription = this.c4eGlobalDataService.tableName$.subscribe(
      {
        next: (name: string) => {
          if (name) {
  ...
  ngOnDestroy(): void {
    ...
    this.uiService.$isSidebarOpen.subscribe(value=>{
