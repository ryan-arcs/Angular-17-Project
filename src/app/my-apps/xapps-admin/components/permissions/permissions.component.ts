...
import { PermissionList, XAppsAdminGlobalDataService } from '../../services';
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  searchTerm = '';
  gridName = 'xapps-admin-permissions';
  ...
  async ngOnInit(): Promise<void> {
    this.subscriptions.add(
      this.xAppsAdminGlobalDataService.permissionsResults$.subscribe({
        next: (data) => {
  ...
  private fetchData(): void {
    ...
    this.xAppsAdminGlobalDataService.getPermissions({
      ...
      submoduleId:
        this.selectedSubmodule?.id === 'all'
          ? undefined
          : parseInt(this.selectedSubmodule?.id),
    });
  }
  ...
  /**
   * Navigates to the edit page for the selected permissions.
   * @param row - The row object containing the permissions data, including its unique ID.
   */
  editRow(row: PermissionList) {
    this.router.navigate([`xapps-admin/permissions/${row.id}/edit`]);
  }

  /**
   * Delete Permissions for the selected permissions.
   * @param row - The row object containing the permissions data, including its unique ID.
   */
  deleteRow(row: PermissionList) {
    if (!row?.id || !row?.permission_name) {
    ...
    modalRef.componentInstance.entity = 'Permission';
    modalRef.componentInstance.HTMLInputElement = true;
    modalRef.componentInstance.app_name = row?.permission_name;
    this.modalRefSubscription = modalRef.closed.subscribe(async (data) => {
      if (data === 'CONFIRM') {
        this.xAppsAdminGlobalDataService.deletePermission({
          id: row?.id.toString(),
        });
      }
    });
  ...
  navigateToAddPermission() {
    this.router.navigate([`xapps-admin/permissions/add`]);
  }
   ...
   */
  async onModuleChange(module: Module) {
    ...
    const moduleId =
      this.selectedModule?.id === 'all'
        ? undefined
        : parseInt(this.selectedModule?.id as string);
    this.fetchData();
    this.submodules =
      await this.xAppsAdminGlobalDataService.getSubmoduleLookup(moduleId);
  }
    ...
    return (
      dropdownList
        ...
        ?.filter(
          (item) =>
            !searchText ||
            item?.sub_module_name?.toLowerCase().includes(searchText),
        ) || []
    );
  }

  /**
   * Set the selected subModule and load permissions for the selected subModule.
   * @param subModule The subModule to select.
   */
  onSubModuleChange(subModule: Submodule) {
