...
import {
  OnboardingResource,
  OnboardingResourceChildTicket,
  OnboardingResourceChildTicketComment,
...
interface GroupedComment {
  date: string;
  comments: OnboardingResourceChildTicketComment[];
}
...
@Component({
  ...
  styleUrl: './onboarding-details.component.scss',
})
export class OnboardingDetailsComponent implements OnInit, OnDestroy {
  options: any;
  onboardingResource?: OnboardingResource;
  onboardingResourceSubscription: Subscription | undefined;
  selectedChildTicket?: OnboardingResourceChildTicket;
  errorMessage = '';
  errorDescription = '';
  formattedStartDate = '';
  selectedTheme = '';
  ...
  ngOnInit(): void {
    const selectedThemeSubcription = this.uiService.activatedTheme$.subscribe({
      next: (theme)=>{
        ...
        this.renderGraph(this.childTickets);
        
      }
    })
    this.subscriptions.add(selectedThemeSubcription);
    this.onboardingResourceSubscription =
      this.resourceTrackerGlobalDataService.onboardingResource$.subscribe({
        next: (onboardingResource: OnboardingResource) => {
          if (onboardingResource?.parentTicket?.id) {
            this.onboardingResource = onboardingResource;
            const childTickets = onboardingResource?.parentTicket?.childTickets;
            this.formattedStartDate =
              this.datePipe.transform(
                onboardingResource?.startDate,
                'MMM dd, yyyy',
              ) || '';
            if (childTickets?.length) {
              ...
              this.errorMessage = messages.error.onboardingChildTicket.notFound;
              this.errorDescription =
                messages.error.onboardingChildTicket.notFoundDescription;
            }
          } else {
            this.errorMessage = messages.error.onboarding.notFound;
            this.errorDescription =
              messages.error.onboarding.notFoundDescription;
          }
        },
      });
  }
  ...
  selectChildTicket(selectedChildTicket: OnboardingResourceChildTicket) {
  ...
  openUserDetailsModal() {
    ...
    columnsModalRef.componentInstance.onboardingResource =
  ...
  showInitials(name: string) {
    return this.uiService.getNameInitials(name);
  }

  groupCommentsByDate(comments?: Array<OnboardingResourceChildTicketComment>) {
  ...
  ngOnDestroy(): void {
    this.onboardingResourceSubscription?.unsubscribe();
