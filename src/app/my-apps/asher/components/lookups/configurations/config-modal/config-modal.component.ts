...
export class ConfigModalComponent {
  ...
  async ngOnInit(): Promise<void> {
    if(this.row.id){
      await this.asherGlobalDataService.getConfigurationsDetails({id: this.row.id});
  ...
  submitNotification(){
    ...
    const apiCall = this.asherGlobalDataService.updateConfigurations({ ...notificationData, id: parseInt(this.row.id) })
